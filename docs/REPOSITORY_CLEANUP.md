...
- The full local Git history scan passed with no findings, and all 29 secret-scanner controls passed. The latest main release notes contained a public Railway deployment UUID that the generic API-key rule flagged. An exact identifier + exact file exception resolves that false positive; replacement UUIDs, other paths, API keys and signing keys beside it remain detected.
