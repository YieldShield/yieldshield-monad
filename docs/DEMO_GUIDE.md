...
This guide is prepared before market activation. Record the video only after `scenario-journey.json` and `reference-journey.json` contain confirmed evidence. Do not fill transaction links or adoption metrics with invented data.
...
Refresh the signed MON price and ensure the API is healthy. Confirm the faucet and exchange inventory, network names, contract addresses, video permissions and source repository visibility. Avoid showing wallet recovery phrases, secret API keys, sign-in screens or unrelated tabs.
...
## Manual acceptance record to complete after activation
