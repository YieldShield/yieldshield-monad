...
import { readCanonicalReceipt } from "./monad-deployment-lib.mjs";

const hash = "0x" + "11".repeat(32),
  otherHash = "0x" + "22".repeat(32),
...
function fixture({
  ...
  const client = {
    getTransactionReceipt: async (options) => {
      calls.push({ method: "receipt", ...options });
      assert.equal(options.hash, hash);
      const result = receipts[receiptIndex++];
      if (result instanceof Error) throw result;
      assert(result, "unexpected extra receipt request");
      return result;
    },
    getBlock: async (options) => {
      calls.push({ method: "block", ...options });
      if (options.blockTag !== undefined) {
        assert.equal(options.blockTag, "latest", "confirmation depth must use the sealed latest block");
        return latest;
      }
      assert.equal(options.blockNumber, 10n, "canonicality must be checked by receipt height");
      const result = blocks[blockIndex++];
      assert(result, "unexpected extra height request");
      return result;
    },
  };
...
test("valid canonical receipt uses a sealed head, rereads receipt/header and returns the fresh receipt", async () => {
  const finalReceipt = { ...receipt, logs: [{ data: "0xbeef" }] };
  const { verify, calls } = fixture({ receipts: [{ ...receipt }, finalReceipt] });
  ...
  assert(
    calls.slice(lastReceipt + 1).some((c) => c.method === "block" && c.blockNumber === 10n),
    "canonical header must be checked after the final receipt",
  );
});
...
test("receipt height must be a positive bigint, not pending or loosely coerced", async () => {
  for (const blockNumber of [null, undefined, 0n, -1n, 10, "10"])
    await assert.rejects(
      fixture({ receipts: [{ ...receipt, blockNumber }] }).verify(),
      `must reject blockNumber ${String(blockNumber)}`,
    );
});
test("zero, missing and malformed receipt block hashes fail closed", async () => {
  for (const invalidHash of [zeroHash, null, undefined, "0x1234", "0x" + "gg".repeat(32)])
    await assert.rejects(
      fixture({ receipts: [{ ...receipt, blockHash: invalidHash }] }).verify(),
      `must reject blockHash ${String(invalidHash)}`,
    );
});
test("receipt block must agree with the canonical header at that height", async () => {
  for (const invalidBlock of [
    { ...block, hash: otherHash },
    { ...block, number: 9n },
  ])
    await assert.rejects(fixture({ blocks: [invalidBlock] }).verify());
});
...
test("final receipt must independently retain successful identity, height and indexed inclusion", async () => {
  for (const changed of [
    ...
    await assert.rejects(fixture({ receipts: [{ ...receipt }, { ...receipt, ...changed }] }).verify());
});
test("a changed canonical header or transaction position after reread rejects a reorg", async () => {
  for (const changed of [
    { ...block, hash: otherHash },
    { ...block, transactions: [hash, otherHash] },
  ])
    await assert.rejects(fixture({ blocks: [{ ...block }, changed] }).verify());
});
test("transient receipt lookup failures propagate without retrying or using stale observations", async () => {
  const failure = Object.assign(new Error("transaction indexing is in progress"), {
    name: "TransactionReceiptNotFoundError",
  });
  const { verify, calls } = fixture({ receipts: [failure] });
  await assert.rejects(verify(), (error) => error === failure);
  assert.deepEqual(calls, [{ method: "receipt", hash }]);
});

test("reviewed failure evidence still requires a sealed canonical reverted receipt", async () => {
  const failed = { ...receipt, status: "reverted" };
  const f = fixture({ receipts: [failed, { ...failed }] });
  assert.equal((await readCanonicalReceipt(f.client, hash, "reviewed-failure", "reverted")).status, "reverted");
  await assert.rejects(readCanonicalReceipt(fixture().client, hash, "not-failed", "reverted"));
  await assert.rejects(
    readCanonicalReceipt(
      fixture({ receipts: [failed, { ...failed, blockHash: zeroHash }] }).client,
      hash,
      "unsealed-failure",
      "reverted",
    ),
