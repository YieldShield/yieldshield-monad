...
import { SequentialDeployment,linkBytecode,assertRuntimeMatches,acquireDeploymentLock } from './monad-deployment-lib.mjs';
...
function fixture({broadcast=true,failFirst=false,pending=false}={}) {
 const path=join(mkdtempSync(join(tmpdir(),'ys-monad-deploy-test-')),'manifest.json');
 const manifest={chainId:10143,contracts:{},transactions:{}};let nonce=0,receipt,attempts=0;const raws=[];
 const client={
...
test('exclusive deployment lock blocks a second process and releases explicitly',()=>{const path=join(mkdtempSync(join(tmpdir(),'ys-monad-lock-test-')),'deployment.lock');const release=acquireDeploymentLock(path);assert.throws(()=>acquireDeploymentLock(path),/EEXIST/);release();const releaseAgain=acquireDeploymentLock(path);releaseAgain();});
