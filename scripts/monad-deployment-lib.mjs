...
const CHAIN_ID=10143, SOURCE_CHAIN_ID=143, DAY=86400n, IMPLEMENTATION_SLOT='0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc';
...
export class SequentialDeployment {
 ...
 async assertSubmissionLimits(request,additionalRequest=false) {
  ...
  const reserved=Object.values(this.manifest.transactions).reduce((n,t)=>n+maximumExecutionCost(t.request)+BigInt(t.request.value??0),0n);
  assert(reserved+(additionalRequest?maximumCost:0n)<=this.spendLimit,'Deployment cumulative maximum fee budget exceeded');
  assert(await this.client.getBalance({address:this.account.address})>=maximumCost+parseEther('0.1'),'Insufficient testnet MON including balance reserve');
 }
 async transaction(id,{to,data,value=0n,kind='call'}) {
  currentStage=id;
  const intent={chainId:CHAIN_ID,from:this.account.address,to:to??null,data,value:String(value)},intentHash=sha(intent);
  ...
  if(!entry) {
   ...
   const gas=(estimated*110n+99n)/100n;assert(gas<=16000000n,'Transaction exceeds conservative Monad gas cap');
   const fees=await this.client.estimateFeesPerGas();
   const request={chainId:CHAIN_ID,type:'eip1559',nonce:latest,to,data,value,gas,maxFeePerGas:fees.maxFeePerGas,maxPriorityFeePerGas:fees.maxPriorityFeePerGas};
  ...
  entry.status='confirmed';entry.receipt={transactionHash:receipt.transactionHash,blockHash:receipt.blockHash,blockNumber:receipt.blockNumber,contractAddress:receipt.contractAddress,gasUsed:receipt.gasUsed,logs:receipt.logs};this.save();
 ...
 async deploy(name,artifactName=name,args=[]) {
  ...
  const data=encodeDeployData({abi:a.abi,bytecode,args});assert((data.length-2)/2<=49152,`${name} exceeds Monad initcode limit`);
