...
assert(
  process.argv.slice(2).every((x) => ["--broadcast", "--prepare"].includes(x)),
  "Unknown option",
);
...
assert.equal(
  ...
  "0xa437345be29ec6802024a8e090e34b621b92e5e2",
  "Wrong dedicated testnet signer",
);
...
const manifest = existsSync(path)
  ...
  : {
      schemaVersion: 1,
      chainId: 10143,
      deploymentKind: "monad-metropolis-testnet-v1",
...
assert.equal(manifest.deployer, account.address);
const release = broadcast ? acquireDeploymentLock(resolve(ROOT, "contracts/.monad-deployment.lock")) : () => {};
...
async function native(id, to, name, fn, args, value) {
  return run.transaction(id, {
    to,
    data: encodeFunctionData({ abi: artifact(name).abi, functionName: fn, args }),
    value,
  });
}
async function pool(id, asset, backing) {
  ...
  const receipt = await run.write(`pool:${id}:create`, factory, "SplitRiskPoolFactory", "createPool", [
    asset.address,
    asset.symbol,
    backing.address,
    backing.symbol,
  ...
  if (broadcast) {
    ...
    address = events[0].args.poolAddress;
  ...
  if (prior) assert.equal(prior.address.toLowerCase(), address.toLowerCase());
    ...
    manifest.pools.push({
      ...
      environment: asset.kind === "synthetic" ? "scenario" : "reference",
      priceKind:
        asset.kind === "synthetic" ? "synthetic" : asset.symbol === "shMON" ? "redemption-nav" : "external-reference",
      createdTx: receipt.transactionHash,
    });
  run.save();
  await run.write(`pool:${id}:approve-seed`, backing.address, backing.artifact, "approve", [
    address,
    50000n * 10n ** 6n,
  ]);
  await run.write(`pool:${id}:seed`, address, "SplitRiskPool", "depositBackingAsset", [
    backing.address,
    50000n * 10n ** 6n,
    50000n * 10n ** 6n,
  ]);
  await run.expect(address, "SplitRiskPool", "POOL_FACTORY", [], factory);
  await run.expect(address, "SplitRiskPool", "requiresStrictProtectedBackingPrice", [], true);
  if (broadcast) {
    const cfg = await run.read(address, "SplitRiskPool", "poolConfig");
    assert.equal(cfg[5], 60n);
    assert.equal(cfg[6], 120n);
  }
  return address;
}
let factory;
try {
  const usd = await run.deploy("TestUSDC", "MonadTestToken", [
    "YieldShield Test USD (no monetary value)",
    "TestUSDC",
    6,
    10000000n * 10n ** 6n,
    account.address,
  ]);
  const lab = await run.deploy("ScenarioMON", "MonadTestToken", [
    "YieldShield Scenario MON (synthetic)",
    "sMON-demo",
    18,
    10000000n * 10n ** 18n,
    account.address,
  ...
  await run.write("vault:seed", vault, "MonadYieldVault", "depositWithMin", [
    500000n * 10n ** 6n,
    500000n * 10n ** 6n,
    account.address,
  ]);
  await run.write("vault:burn-seed", vault, "MonadYieldVault", "transfer", [
  ...
  const reference = await run.deploy("ReferenceFeed", "MonadReferenceFeed", [
    config.pyth.address,
    config.pyth.monUsdFeedId,
    wmon,
    config.externalTokens.shMON,
    usd,
  ]);
  const staking = await run.deploy("StakingRouter", "MonadStakingRouter", [config.externalTokens.shMON]);
  const exchangeOracle = await run.deploy("ScenarioExchangeOracle", "MonadAssetOracle", [
    usd,
    [lab, vault],
    [scenario, nav],
  ]);
  ...
  for (const role of ["PROPOSER_ROLE", "EXECUTOR_ROLE", "CANCELLER_ROLE"])
    await run.write(`timelock:${role}`, timelock, "YSTimelockController", "grantRole", [
  ...
  for (const [name, config] of Object.entries(modules)) {
    ...
    for (const group of Object.values(config.modules)) {
      ...
      if (group.contract === "BasePoolInitializeModule")
        target = await run.deploy("MonadPoolInitializeModule", "MonadPoolInitializeModule", [
          target,
          wmon,
          configExternalShMon(),
          lab,
  ...
  function configExternalShMon() {
    return config.externalTokens.shMON;
  }
  factory = await run.deploy("Factory", "ERC1967Proxy", [
  ...
  for (const a of [assets[0], assets[2]])
    await run.write(`factory:strict:${a.id}`, factory, "SplitRiskPoolFactory", "setTokenRequiresStrictProtectedPrice", [
      a.address,
      true,
    ]);
  ...
  for (const [id, token, name, amount] of [
    ["usd", usd, "MonadTestToken", 2000000n * 10n ** 6n],
    ["mon", lab, "MonadTestToken", 100000n * 10n ** 18n],
    ["vault", vault, "MonadYieldVault", 10000n * 10n ** 6n],
  ])
    await run.write(`exchange:fund:${id}`, token, name, "transfer", [exchange, amount]);
  for (const [id, token, amount] of [
    ["usd", usd, 5000000n * 10n ** 6n],
    ["mon", lab, 500000n * 10n ** 18n],
  ])
    await run.write(`faucet:fund:${id}`, token, "MonadTestToken", "transfer", [faucet, amount]);
  await run.write("faucet:config", faucet, "ConfigurableTokenFaucet", "setTokens", [
    [usd, lab],
    [10000n * 10n ** 6n, 25n * 10n ** 18n],
  ]);
  ...
  run.save();
  if (!broadcast)
    atomicJson(resolve(ROOT, "artifacts/local/monad-prepare.json"), {
      chainId: 10143,
      steps: run.plan,
      note: "Prepared only. No transactions signed or broadcast.",
    });
  console.log(
    `Monad ${broadcast ? "deployment" : "preparation"} complete: ${run.plan.length || Object.keys(manifest.transactions).length} steps, ${manifest.pools.length} scenario pools.`,
  );
} finally {
  release();
}
