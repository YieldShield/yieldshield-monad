...
async function main() {
  ...
  const api = audit(path.join(repo, "services/monad"));
  checkAudit(api.report, api.lock);
  ...
  console.log(`Dependency check passed: ${accepted} root findings from ${advisoryCount} reviewed advisories; API clean.`);
