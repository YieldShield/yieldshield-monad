...
/// @notice Constructor-pinned routing for demo stocks, crypto and genuinely backed vault shares.
/// @dev The execution venue reads the same source feeds that price the corresponding pools.
contract AlphaAssetOracle {
    ...
    constructor(address quote, address[] memory tokens, address[] memory feeds) {
        if (block.chainid != 84532 || tokens.length == 0 || tokens.length > 8 || tokens.length != feeds.length) {
    ...
    function getPrice(address token) external view returns (uint256) {
        if (block.chainid != 84532 || source[token] == address(0)) revert InvalidConfiguration();
