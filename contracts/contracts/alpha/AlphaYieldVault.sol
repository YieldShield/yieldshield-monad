...
/// @notice Base Sepolia vault shares backed by real balances of valueless test assets.
...
contract AlphaYieldVault is ERC4626, ReentrancyGuard {
    ...
    {
        if (block.chainid != 84532) revert DemoChainOnly();
        if ((underlying.codehash != keccak256(type(BaseSepoliaAlphaToken).runtimeCode) && underlying.codehash != keccak256(type(AlphaCryptoToken).runtimeCode))) revert InvalidUnderlying();
    }
