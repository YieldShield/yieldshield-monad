...
import { AlphaScenarioOracle } from "./AlphaScenarioOracle.sol";

/// @notice Inventory-funded exchange for valueless demo stock/USD tokens on Base Sepolia.
...
contract AlphaStockExchange is ReentrancyGuard {
    ...
    uint256 public constant maxStockAmount = 25e8;
    uint256 public constant MAX_DEADLINE_DELAY = 10 minutes;
    AlphaScenarioOracle public immutable oracle;
    ...
    constructor(address oracle_) {
        _requireDemoChain();
        if (oracle_.codehash != keccak256(type(AlphaScenarioOracle).runtimeCode)) revert InvalidOracle();
        oracle = AlphaScenarioOracle(oracle_);
        quoteToken = oracle.quoteToken();
        for (uint256 i; i < oracle.stockCount(); ++i) {
            supportedStock[oracle.stockTokens(i)] = true;
        }
    }

    /// @notice Exact stock quantity (8 decimals); total demo USD amount (6 decimals) includes fee.
    ...
    {
        _requireDemoChain();
        if (!supportedStock[stock] || stockAmount == 0 || stockAmount > maxStockAmount) revert InvalidTrade();
        price = oracle.getPrice(stock);
        uint256 notional = Math.mulDiv(stockAmount, price, 1e10, buy ? Math.Rounding.Ceil : Math.Rounding.Floor);
