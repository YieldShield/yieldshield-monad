...
contract BaseFactoryCreateModule is
    Initializable,
    ...
    UUPSUpgradeable
{
     ...
     */
    function removeToken(address token) external onlyGovernance { revert BaseModuleSelectorUnavailable(); }
    ...
    // Slither reentrancy-eth false positive: guarded by nonReentrant and/or governance-only access (or internal, reached only via such guarded entrypoints); external calls are to trusted protocol contracts.
    // slither-disable-next-line reentrancy-eth
    function deactivatePool(address pool) external onlyGovernance nonReentrant { revert BaseModuleSelectorUnavailable(); }
    ...
    // Slither reentrancy-eth false positive: guarded by nonReentrant and/or governance-only access (or internal, reached only via such guarded entrypoints); external calls are to trusted protocol contracts.
    // slither-disable-next-line reentrancy-eth
    function deactivateDustPool(address pool) external onlyGovernance nonReentrant { revert BaseModuleSelectorUnavailable(); }
    ...
    // Slither reentrancy-eth false positive: guarded by nonReentrant and/or governance-only access (or internal, reached only via such guarded entrypoints); external calls are to trusted protocol contracts.
    // slither-disable-next-line reentrancy-eth
    function deactivateProtectorOnlyPool(address pool) external onlyGovernance nonReentrant { revert BaseModuleSelectorUnavailable(); }
    ...
    // Slither reentrancy-eth false positive: guarded by nonReentrant and/or governance-only access (or internal, reached only via such guarded entrypoints); external calls are to trusted protocol contracts.
    // slither-disable-next-line reentrancy-eth
    function closePool(address pool) external nonReentrant { revert BaseModuleSelectorUnavailable(); }
    ...
    // Slither reentrancy-eth false positive: guarded by nonReentrant and/or governance-only access (or internal, reached only via such guarded entrypoints); external calls are to trusted protocol contracts.
    // slither-disable-next-line reentrancy-eth
    function closePoolTo(address pool, address bondRecipient) external nonReentrant { revert BaseModuleSelectorUnavailable(); }
