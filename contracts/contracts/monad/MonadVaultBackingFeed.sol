...
/// @notice Monad testnet backing-share price at checked LIVE redeemable NAV.
...
contract MonadVaultBackingFeed {
    error InvalidConfiguration();
    ERC4626OracleFeed public navFeed;
    address public backingVault;
    constructor(address feed, address vault) {
        if (block.chainid != 10143 || feed.code.length == 0 || vault.code.length == 0) revert InvalidConfiguration();
    ...
    function getPrice(address token) public view returns(uint256) {
        if (block.chainid != 10143 || token != backingVault) revert InvalidConfiguration();
    ...
    function _supported(address token) private view returns(bool) {
        return block.chainid == 10143 && token == backingVault && navFeed.supportsStrictProtectedPrice(token);
    }
}
