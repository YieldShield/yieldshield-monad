...
/// @notice Synthetic, predictable prices for valueless Monad testnet demo tokens only.
...
contract MonadScenarioOracle is IPriceOracle {
    error DemoChainOnly();
    ...
    constructor(address quoteToken_, address[] memory stocks, uint256[] memory basePrices, uint64 cycleSeconds_) {
        ...
        for (uint256 i; i < stocks.length; ++i) {
            ...
            if (basePrice[token] != 0 || basePrices[i] < 1000 || basePrices[i] > MAX_BASE_PRICE) {
                revert InvalidConfiguration();
            }
            uint8 tokenDecimals = MonadTestToken(token).decimals();
    ...
    function description() external pure returns (string memory) {
        return "YieldShield synthetic Monad testnet demo prices";
    }
    ...
    function _requireDemoToken(address token, uint8 expectedDecimals) private view {
        if (
            token.codehash != keccak256(type(MonadTestToken).runtimeCode)
                || MonadTestToken(token).decimals() != expectedDecimals
        ) revert InvalidConfiguration();
    }
}
