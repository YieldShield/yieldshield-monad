...
import { MonadExpandedInitializeModule } from "../../contracts/monad/MonadExpandedInitializeModule.sol";
...
contract MonadExpandedLifecycleTest is Test {
 ...
 function buildPool(bool useStock) internal {
  ...
  MonadAssetRegistry.AssetConfig[] memory assets=new MonadAssetRegistry.AssetConfig[](3);
  assets[0]=MonadAssetRegistry.AssetConfig(address(weth),address(price),address(weth),1);
  assets[1]=MonadAssetRegistry.AssetConfig(address(stock),address(price),address(stock),1);
  assets[2]=MonadAssetRegistry.AssetConfig(address(vu),address(nav),address(vu),2);
  MonadAssetRegistry assetRegistry=new MonadAssetRegistry(assets);
  BasePoolRouter router=new BasePoolRouter(original.adminModule(),original.depositsModule(),original.feesModule(),address(new MonadExpandedInitializeModule(original.initializeModule(),address(assetRegistry))),original.partialexitModule(),original.protectorModule(),original.shieldexitModule(),original.viewsModule());
