...
import { AlphaCryptoToken } from "../contracts/alpha/AlphaCryptoToken.sol";
import { BaseSepoliaAlphaToken } from "../contracts/alpha/BaseSepoliaAlphaToken.sol";
import { AlphaCryptoScenarioOracle } from "../contracts/alpha/AlphaCryptoScenarioOracle.sol";
import { AlphaAssetOracle } from "../contracts/alpha/AlphaAssetOracle.sol";
import { AlphaAssetExchange } from "../contracts/alpha/AlphaAssetExchange.sol";
import { AlphaVaultBackingFeed } from "../contracts/alpha/AlphaVaultBackingFeed.sol";
import { AlphaYieldVault } from "../contracts/alpha/AlphaYieldVault.sol";
import { ERC4626OracleFeed } from "../contracts/oracles/ERC4626OracleFeed.sol";
import { CompositeOracle } from "../contracts/oracles/CompositeOracle.sol";
import { AlphaCryptoInitializeModule } from "../contracts/alpha/AlphaCryptoInitializeModule.sol";
...
contract AlphaCryptoVaultTest is Test {
 BaseSepoliaAlphaToken usd; BaseSepoliaAlphaToken stock; AlphaCryptoToken weth; AlphaCryptoToken btc;
 AlphaYieldVault vu; AlphaYieldVault vw; AlphaCryptoScenarioOracle price; ERC4626OracleFeed nav;
 AlphaAssetExchange exchange; SplitRiskPool pool; AlphaVaultBackingFeed backingFeed; address constant USER=address(0xA11CE);
 ...
 function setUp() public {
  vm.chainId(84532);
  usd=new BaseSepoliaAlphaToken("Test USD","TestUSDC",6,10000000e6,address(this));
  weth=new AlphaCryptoToken("Test WETH","tWETH",18,1000000e18,address(this));
  btc=new AlphaCryptoToken("Test cbBTC","tcbBTC",8,1000000e8,address(this));
  stock=new BaseSepoliaAlphaToken("Test Apple","tAAPLc",8,1000000e8,address(this));
  ...
  price=new AlphaCryptoScenarioOracle(address(usd),assets,prices,7200);
  vu=new AlphaYieldVault(address(usd),"Test USD Yield Vault","vUSDC");
  vw=new AlphaYieldVault(address(weth),"Test WETH Yield Vault","vWETH");
  ...
  exchange=new AlphaAssetExchange(address(new AlphaAssetOracle(address(usd),all,feeds)));
 ...
 function testFundedYieldCannotExceedDemoReferenceBudget() public {
  ...
  vm.expectRevert(AlphaYieldVault.InvalidYield.selector);vu.fundTestYield(500e6);
 }
 function buildPool(bool useStock) internal {
  ...
  BasePoolRouter router=new BasePoolRouter(original.adminModule(),original.depositsModule(),original.feesModule(),address(new AlphaCryptoInitializeModule(original.initializeModule(),address(vw),address(vu))),original.partialexitModule(),original.protectorModule(),original.shieldexitModule(),original.viewsModule());
  ...
  backingFeed=new AlphaVaultBackingFeed(address(nav),address(vu));
 ...
 function testExistingStockTokenAcceptsVaultCollateral() public {
